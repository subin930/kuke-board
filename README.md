# Kuke Board

Spring Boot 기반 게시판 프로젝트

<br />

## 🔧 Tech Stack
- Java 21
- Spring Boot 3.3.2
- Spring Data JPA
- MySQL
- Redis 7.4

<br />

## 📚 Study Notes
- [대규모 시스템 서버 인프라 기초](docs/study/large-scale-system.md)
- [분산 관계형 데이터베이스](docs/study/distributed-relational-db.md)
- [Primary Key 생성 전략 & Snowflake](docs/study/primary-key-generation-strategy.md)
- [데이터베이스 인덱스](docs/study/db-index.md)
- [효율적인 페이지네이션 구현](docs/study/pagination.md)
- [무한 스크롤 구현](docs/study/infinite-scroll.md)
- [테스트 데이터 생성하는 코드 작성하기](docs/spring/study/generate-test-data.md)
- [AccessLevel 이해하기](docs/spring/study/access-level.md)
- [스프링부트에서의 테스트](docs/spring/study/test.md)
- [테스트 클래스에서 API 테스트하기](docs/spring/study/test-rest-client.md)
- [무한 depth 댓글 설계 및 구현하기](docs/study/infinite-depth-comments.md)
- [동시 쓰기 요청을 데이터 유실/장애 없이 해결하기(비관적 락, 낙관적 락, 비동기 순차 처리)](docs/study/lock-type.md)
- [조회수 구현 설계와 Redis, 어뷰징 방지 ](docs/study/view-and-redis.md)
<br />

## 🧠 What I Learned
- [java.lang.ClassNotFoundException 해결하기](docs/spring/trouble-shooting/class-not-found-exception-solution.md)
- [Docker MySQL + Spring Boot 연결 오류 해결기 (feat. 포트 충돌)](docs/spring/trouble-shooting/port-collision.md)

<br />

## 참고 자료(Reference)

이 프로젝트는 아래 강의를 참고하여 작성했습니다.
- 강의명: [스프링부트로 직접 만들면서 배우는 대규모 시스템 설계 - 게시판]([강의 링크](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81%EB%B6%80%ED%8A%B8%EB%A1%9C-%EB%8C%80%EA%B7%9C%EB%AA%A8-%EC%8B%9C%EC%8A%A4%ED%85%9C%EC%84%A4%EA%B3%84-%EA%B2%8C%EC%8B%9C%ED%8C%90))
- 출처: 쿠케
